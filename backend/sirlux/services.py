...
def verificar_disponibilidad(fecha, hora_inicio, hora_fin, reserva_id_ignore=None):
    ...
    Usa objetos datetime para contemplar cierres después de medianoche.
    (Ignora hora de inicio/cierre operativo, a petición del usuario, 
    permitiendo reservas desde las 10:00 a 21:00 sin importar si cierra a las 12).
    """
    config = ConfiguracionSistema.get_solo()
    ...
    if dt_fin_nueva <= dt_inicio_nueva: # Cruza la medianoche
        dt_fin_nueva += timedelta(days=1)

    # Buscar reservas que puedan traslapar en el rango extendido
    ...
    for r in reservas_rango:
        ...
        # Hay traslape si los rangos se superponen (Inicio1 < Fin2) AND (Fin1 > Inicio2)
